...
class shoppinglistsWindow(Screen):
    ...
    def on_pre_enter(self):
        ...
        button1=Button(text='+',size_hint=(.1,.1),pos_hint ={'x':.4, 'y':.0},
				        ...
				        color =(1, 1, 1, 1),bold=True)
        button1.bind(on_press=self.createnew)
        for c in list(self.ids.grid.children):
            if isinstance(c, Button) and c.text=='+' : self.ids.grid.children.remove(c)
        self.ids.grid.add_widget(button1)
        #shopping list label
        label= Label(text ="Shopping List!", font_size ='20sp',
            color =[0, 0, 0, 1],size_hint = (0.2, 0.1),
            pos_hint ={"x":0.4,"y":0.92})
        self.ids.float.add_widget(label)
        #back button
        ...
        self.ids.float.add_widget(backbtn)
        #settings button
        settings=Button(text='SS',size_hint=(0.03,0.02),pos_hint ={"x":0.93,"y":0.96} ,
                        background_normal= 'Settingsicon.jpg',
                        background_down= 'Settingsicon.jpg',mipmap= True)
        settings.bind(on_press=self.slsettings) 
        self.ids.float.add_widget(settings)
    def createnew(self,event):
        btn = Button(text="New Note"+str(len(self.ids.grid.children)),size_hint=(.8,.1),pos_hint ={'x':.1, 'y':.65},
                        background_color =(0, 0, 0, 1),
				        color =(1, 1, 1, 1),bold=True) 
        btn.bind(on_press=self.addn) 
        self.ids.grid.add_widget(btn) 
    ...
    def addn(self, event):
        ...
        print("Plus Clicked:" + event.text)
        if event.text.startswith("New Note"):
            App.get_running_app().notefileName=event.text+".csv"
        else:
            App.get_running_app().notefileName=None
        # read data from file and set to name , list name
        try:
            App.get_running_app().listname=pd.read_csv(event.text+".csv")
        except:
            pass
    def back(self,event):
        if App.get_running_app().notefileName!=None:
            df = pd.DataFrame({name:App.get_running_app().name, listname:App.get_running_app().listname})
            df.to_csv(App.get_running_app().notefileName, header=False, index=False)
        sm.current='homepage'
...
class homepageWindow(Screen):
    def _init_(self, **kwargs):
        super(homepageWindow, self)._init_(**kwargs)
